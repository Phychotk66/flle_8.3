...
class TwintCw_AbstractOrderContext extends Customweb_Payment_Authorization_OrderContext_AbstractDeprecated implements Customweb_Payment_Authorization_IOrderContext {
	...
	public function __construct(Customweb_Payment_Authorization_IPaymentMethod $paymentMethod, array $invoiceItems, $shippingAddress, $billingAddress, $shippingMethodName, $orderTotal, $currencyCode, $languageCode) {
		...
		else if (is_null($this->shippingAddress->cFirma) && !is_null($this->billingAddress->cFirma)) {
			// Set the company name as it is needed but not asked by the form.
			$this->shippingAddress->cFirma = $this->billingAddress->cFirma;
		}

		$customerId = $this->getCustomerId();

		if ($customerId !== null) {
			$anzahl_obj = TwintCw_VersionHelper::getInstance()->getDb()->executeQuery("select count(*) as anz from tbestellung where kKunde=" . $customerId . " and (cStatus=\"" . BESTELLUNG_STATUS_BEZAHLT . "\" or cStatus=\"" . BESTELLUNG_STATUS_VERSANDT . "\")", 1);
		...
		if (!isset($_SESSION['twintcw_checkout_id'])) {
			$_SESSION['twintcw_checkout_id'] = array();
		}
		if (!isset($_SESSION['twintcw_checkout_id'][$paymentMethod->getPaymentMethodName()])) {
			$_SESSION['twintcw_checkout_id'][$paymentMethod->getPaymentMethodName()] = Customweb_Core_Util_Rand::getUuid();
		}
		$this->checkoutId = $_SESSION['twintcw_checkout_id'][$paymentMethod->getPaymentMethodName()];
	}
	...
	public function getShippingMethod() {
		if ($this->shippingMethodName !== null) {
			...
			return TwintCw_Language::_('No Shipping');
		}
	}
	...
	protected function getCustomerDateOfBirthByCustomer(Kunde $customer) {
		if ($customer !== null) {
			if (!empty($customer->dGeburtstag) && $customer->dGeburtstag != '0000-00-00' && $customer->dGeburtstag != '00.00.0000' && $customer->dGeburtstag != '_DBNULL_') {
				try {
	...
	protected function getSessionLanguageCode() {
		if(isset($_SESSION['cISOSprache']) && !empty($_SESSION['cISOSprache'])) {
			return TwintCw_VersionHelper::getInstance()->convertISO2ISO639($_SESSION['cISOSprache']);
		}
		else {
			return TwintCw_Util::getDefaultLanguageCode();
		}
	}

	protected static function encodeStringToUTF8($string) {
		file_put_contents('/zzz.log', print_r(
			[
				'string' => $string,
				'encoding' => mb_detect_encoding($string)
			], true
		), FILE_APPEND);
		return Customweb_Core_Charset_UTF8::fixCharset($string);
		return $string;
	}
}