 ...
 *
 */
class TwintCw_Adapter_Configuration implements Customweb_Payment_IConfigurationAdapter{
	
	/**
	 * @var TwintCw_SettingApi
	 */
	...
	private static function getSetting() {
		if (self::$config === null) {
			self::$config = new TwintCw_SettingApi();
		}
		return self::$config;
	}
	...
	public function getLanguages($currentStore = false) {
		$rs = TwintCw_Util::getDriver()->query('SELECT cNameEnglisch FROM tsprache');
		$languages = array();
