...
require_once 'TwintCw/MutableConstantApi.php';
...
class TwintCw_Cron {
	...
	public function run() {
		...
		if (!$this->isOtherCronRunning()) {
			$config = new TwintCw_SettingApi();
			TwintCw_MutableConstantApi::setConstant(self::CRON_MUTEX_NAME, 0);
		}
		$cronProcessor = new Customweb_Cron_Processor(TwintCw_Util::getContainer());
		$cronProcessor->run();
	}
	...
	private function isOtherCronRunning() {
		$mutex = TwintCw_MutableConstantApi::getConstant(self::CRON_MUTEX_NAME);
		if ($mutex === null) {
			TwintCw_MutableConstantApi::setConstant(self::CRON_MUTEX_NAME, time());
			return false;
		}
