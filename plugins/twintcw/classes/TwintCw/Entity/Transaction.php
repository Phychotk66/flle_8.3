...
/**
 *
 * @Entity(tableName = 'xplugin_twintcw_transactions')
 * @Filter(name = 'loadByOrderInternalId', where = 'orderInternalId = >orderInternalId', orderBy = 'orderInternalId')
 */
final class TwintCw_Entity_Transaction extends Customweb_Payment_Entity_AbstractTransaction
{
	...
	public function onBeforeSave(Customweb_Database_Entity_IManager $entityManager){
		...
		if ($amountNew > 0.00001) {
			$db->insert('tzahlungseingang', array(
				...
				'>cZahlungsanbieter' => 'TWINT: ' . $paymentMethodName,
	...
	protected function authorize(Customweb_Database_Entity_IManager $entityManager) {
		...
		$reloadedTransaction = $entityManager->fetch('TwintCw_Entity_Transaction', $this->getTransactionId(), false);
		...
		if (empty($kBestellNr) || empty($kBestellung)) {
			TwintCw_Log::add("The order has not been created correctly. We cannot "
		...
		//mail
		$oPlugin = TwintCw_Util::getPluginObject();
		$settings = $oPlugin->oPluginZahlungsmethodeAssoc_arr[TwintCw_Util::getPaymentMethodModuleId($this->getPaymentMachineName())];
		if (($settings->nMailSenden & ZAHLUNGSART_MAIL_EINGANG) && strlen($kunde->cMail) > 0) {
			...
			$mail = $mail->createFromTemplateID(MAILTEMPLATE_BESTELLUNG_BEZAHLT, $obj);
			$mailer->send($mail);
		}
	...
	public function getPaymentMethod() {
		if ($this->payment_method_reference === null) {
			$this->payment_method_reference = TwintCw_PaymentMethod::getInstanceByPaymentMethodName($this->getPaymentMachineName());
		}
	...
	public function getJTLSuccessUrl() {
		...
		if ($row !== false && count($row) > 0) {
			...
			TwintCw_VersionHelper::getInstance()->getDb()->insertRow('tbestellid',$bestellid);
			$i = $bestellid->cId;
		}

		$url = TwintCw_Util::getStoreBaseUrl() . '/bestellabschluss.php?i=' . $i;
	...
	public static function getGridQuery() {
		return 'SELECT * FROM xplugin_twintcw_transactions WHERE ${WHERE} ${ORDER_BY} ${LIMIT}';
	}
	...
	public static function getTransactionsByOrderInternalId($id) {
		$rs = TwintCw_Util::getEntityManager()->searchByFilterName('TwintCw_Entity_Transaction', 'loadByOrderInternalId', array('>orderInternalId' => $id));
		if (count($rs) > 0) {
			return $rs;
		}
		else {
	...
	public static function getTransactionsWithChangedOrderStatus($limit = 50) {
		...
		$transactions = array();
		$statment = $db->query("SELECT t.transactionId FROM xplugin_twintcw_transactions AS t, tbestellung AS b WHERE t.orderInternalId = b.kBestellung AND t.orderStatus != b.cStatus LIMIT 0," . (int)$limit);
