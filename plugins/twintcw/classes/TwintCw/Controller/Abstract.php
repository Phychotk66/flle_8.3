...
abstract class TwintCw_Controller_Abstract {
	...
	public function __construct() {
		...
		$this->setSiteName(TwintCw_Language::_("Payment"));
		
		$this->addBreadcrumbItem(TwintCw_VersionHelper::getInstance()->getTranslation('startpage', 'breadcrumb'), TwintCw_Util::getStoreBaseUrl());
	}
	...
	protected function getUrl(array $parameters = array(), $action = null, $controller = null) {
		...
		return TwintCw_Util::getUrl($controller, $action, $parameters);
	}
	...
	protected function display($templateFile) {
		...
		$asset = TwintCw_Util::getAssetResolver()->resolveAssetStream($templateFile . '.tpl');
	...
	protected function getAdditionalResourcesHtml() {
		$output = '';
		$output .= TwintCw_Util::buildResourceHtml($this->getJavaScriptFiles(), 'js');
		$output .= TwintCw_Util::buildResourceHtml($this->getCssFiles(), 'css');
