...
require_once 'TwintCw/Installer.php';
require_once 'TwintCw/Grid/Renderer.php';
require_once 'TwintCw/Controller/AbstractBackend.php';
require_once 'TwintCw/Language.php';
require_once 'TwintCw/Grid/TransactionStatusColumn.php';
...
class TwintCw_Controller_Transaction extends TwintCw_Controller_AbstractBackend {
	...
	public function listAction(){
		...
		$loader->addColumn(new Customweb_Grid_Column('transactionId', '#'))->addColumn(
				...
				new TwintCw_Grid_TransactionStatusColumn('status', 'Authorization Status'))->addColumn(
				new Customweb_Grid_Column('createdOn', 'Erstellungsdatum', 'DESC'))->addColumn(
				new TwintCw_Grid_TransactionActionColumn('actions'));
		
		$renderer = new TwintCw_Grid_Renderer($loader, $this->getUrl());
	...
	public function viewAction(){
		...
		if ($transaction->getOrderInternalId() > 0) {
			$relatedTransactions = TwintCw_Entity_Transaction::getTransactionsByOrderInternalId($transaction->getOrderInternalId());
		}
		...
		if (is_object($transaction->getTransactionObject()) && !$transaction->isAcceptUncertain() &&
			...
			$buttonName = TwintCw_Language::_('Accept Uncertain');
		...
		if (is_object($transaction->getTransactionObject())) {
			$this->assign('update_url', $this->getUrl(array(
				'transaction_id' => $transaction->getTransactionId() 
			), 'manualUpdate'));
		}
	...
	public function captureAction(){
		...
		if (isset($_POST['quantity'])) {
			...
			if (count($captureLineItems) > 0) {
				$adapter = TwintCw_Util::getContainer()->getBean('Customweb_Payment_BackendOperation_Adapter_Service_ICapture');
				...
				try {
					...
					$this->setSuccessMessage(TwintCw_Language::_('Capture was successful.'));
					$this->viewAction();
					return;
				}
				catch (Exception $e) {
	...
	public function refundAction(){
		...
		if (isset($_POST['quantity'])) {
			...
			if (count($refundLineItems) > 0) {
				$adapter = TwintCw_Util::getContainer()->getBean('Customweb_Payment_BackendOperation_Adapter_Service_IRefund');
				...
				try {
					...
					TwintCw_Util::getEntityManager()->persist($transaction);
					$this->setSuccessMessage(TwintCw_Language::_('Refund was successful.'));
					$this->viewAction();
					return;
				}
				catch (Exception $e) {
	...
	public function cancelAction(){
		$transaction = $this->getTransactionFromRequest();
		$adapter = TwintCw_Util::getContainer()->getBean('Customweb_Payment_BackendOperation_Adapter_Service_ICancel');
		...
		try {
			...
			TwintCw_Util::getEntityManager()->persist($transaction);
			$this->setSuccessMessage(TwintCw_Language::_('Cancel was successful.'));
		}
		catch (Exception $e) {
			$this->setErrorMessage($e->getMessage());
			TwintCw_Util::getEntityManager()->persist($transaction);
	...
	public function manualUpdateAction(){
		$transaction = $this->getTransactionFromRequest();
		$updateAdapter = TwintCw_Util::getContainer()->getBean('Customweb_Payment_Update_IAdapter');
		if (!($updateAdapter instanceof Customweb_Payment_Update_IAdapter)) {
			throw new Exception("No adapter with interface 'Customweb_Payment_Update_IAdapter' provided.");
		}
		
		/* @var $transaction TwintCw_Entity_Transaction */
		try {
			$transactionObject = $transaction->getTransactionObject();
			$updateAdapter->updateTransaction($transactionObject);
			TwintCw_Util::getTransactionHandler()->persistTransactionObject($transactionObject);
			$this->setSiteName(TwintCw_Language::_('Update was successful.'));
		}
		catch (Exception $e) {
	...
	private function getTransactionFromRequest(){
		...
		$transaction = TwintCw_Entity_Transaction::loadById($_REQUEST['transaction_id']);
