...
class TwintCw_Controller_AbstractBackend extends TwintCw_Controller_Abstract {
	...
	protected function display($templateFile) {
		$path = PFAD_ROOT . '/plugins/twintcw/template/' . $templateFile . '.tpl';
		$this->render($path);
	}
	...
	protected function getUrl(array $parameters = array(), $action = null, $controller = null) {
		...
		foreach (TwintCw_Util::getPluginObject()->oPluginAdminMenu_arr as $menu) {
		...
		return TwintCw_Util::getBackendUrl($controller, $action, $parameters);
	}
