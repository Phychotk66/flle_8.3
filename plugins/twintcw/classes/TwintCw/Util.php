...
require_once 'TwintCw/NiceDbWrapper.php';
...
final class TwintCw_Util {
	...
	public static function getContainer() {
		if (self::$container === null) {
			...
			$provider
				...
				->add('databaseTransactionClassName', 'TwintCw_Entity_Transaction')
	...
	public static function getDriver() {
		if (self::$driver === null) {
			$wrapper = new TwintCw_NiceDbWrapper(TwintCw_VersionHelper::getInstance()->getDb());
	...
	public static function getPluginObject() {
		if (self::$pluginObject === null) {
			if (isset($GLOBALS['oPlugin']) && $GLOBALS['oPlugin'] !== null && $GLOBALS['oPlugin']->cPluginID == 'twintcw') {
				self::$pluginObject = $GLOBALS['oPlugin'];
			}
			else {
				self::$pluginObject = Plugin::getPluginById('twintcw');
			}

			if (self::$pluginObject === null || TwintCw_VersionHelper::getInstance()->getPluginKey(self::$pluginObject) === null) {
	...
	public static function getPaymentMethodModuleId($paymentMethodMachineName) {
		...
		return 'kPlugin_' . TwintCw_VersionHelper::getInstance()->getPluginKey($plugin) . '_' . self::getPaymentMethodPrefix() . $externalId;
	}
	...
	public static function getPaymentMethodPrefix() {
		if (self::$paymentMethodPrefixName === null) {
			self::$paymentMethodPrefixName = preg_replace('/[^0-9a-z]+/i', '', strtolower('TWINT'));
		}
	...
	public static function getDefaultLanguageCode() {
		$oSprache = TwintCw_VersionHelper::getInstance()->getDb()->executeQuery("SELECT kSprache, cISO FROM tsprache WHERE cShopStandard = 'Y'", 1);
		if($oSprache->kSprache > 0) {
			return TwintCw_VersionHelper::getInstance()->convertISO2ISO639($oSprache->cISO);
		}
		else {
			return 'de';
		}
	...
	public static function getEndpointFileUrl() {
		return trim(TwintCw_Util::getStoreBaseUrl(), '/ ') . '/twintcw.php';
	}
	...
	public static function getBackendUrl($controller, $action = null, array $parameters = array()) {
		...
		if (isset($_REQUEST['kPlugin'])) {
			...
			$pluginId = TwintCw_VersionHelper::getInstance()->getPluginKey(self::getPluginObject());
		}
		...
		$url = trim(TwintCw_Util::getStoreBaseUrl(), '/ ') . '/' . $admin. 'plugin.php?kPlugin=' . $pluginId;
	...
	public static function buildResourceHtml(array $files, $type) {
		...
		foreach ($files as $file) {
			...
			if ($type == 'js') {
				$output .= TwintCw_VersionHelper::getInstance()->buildScriptTag($relativeFilePath);
			}
			else {
	...
	public static function getJtlAdapterByPaymentAdapter(Customweb_Payment_Authorization_IAdapter $paymentAdapter) {
		...
		$adapters = self::getContainer()->getBeansByType('TwintCw_Adapter_IAdapter');
	...
	/**
	 * @return TwintCw_EndpointAdapter
	 */
	public static function getEndpointAdapter() {
		return self::getContainer()->getBean('TwintCw_EndpointAdapter');
	}
	...
	public static function getAssetResolver() {
		if (self::$resolver === null) {
			$currentTemplate = TwintCw_VersionHelper::getInstance()->getTemplateDirectory();
			$templatePath = PFAD_ROOT . PFAD_TEMPLATES . $currentTemplate . '/';
			$templateUrl = trim(TwintCw_Util::getStoreBaseUrl(), '/ ') . '/' . PFAD_TEMPLATES . $currentTemplate . '/';

			$pluginPath = PFAD_ROOT . '/plugins/twintcw/';
			$pluginUrl = trim(TwintCw_Util::getStoreBaseUrl(), '/ ') . '/plugins/twintcw/';

			self::$resolver = new Customweb_Asset_Resolver_Composite(array(
				new Customweb_Asset_Resolver_Simple(
						$templatePath . 'twintcw/snippets/',
						$templateUrl . 'twintcw/snippets/',
						array('application/x-smarty')
				),
				new Customweb_Asset_Resolver_Simple(
						$templatePath . 'twintcw/',
						$templateUrl . 'twintcw/',
						array('application/x-smarty')
				),
				new Customweb_Asset_Resolver_Simple(
						$templatePath . 'twintcw/css/',
						$templateUrl . 'twintcw/css/',
						array('text/css')
				),
				new Customweb_Asset_Resolver_Simple(
						$templatePath . 'twintcw/js/',
						$templateUrl . 'twintcw/js/',
						array('application/javascript')
				),
				new Customweb_Asset_Resolver_Simple(
						$templatePath . 'twintcw/images/',
						$templateUrl . 'twintcw/images/',
						array('image/png')
				),
				new Customweb_Asset_Resolver_Simple(
