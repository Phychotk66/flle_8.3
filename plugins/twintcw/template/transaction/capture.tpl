...
<?php if ($transaction->getTransactionObject()->isPartialCapturePossible()):?>
	<h3><?php echo TwintCw_Language::_('Partial Capture'); ?></h3>
	<form action="<?php $captureConfirmUrl; ?>" method="POST" class="twintcw-line-item-grid" id="capture-form">
		...
		<table class="table">
			...
			<tfoot>
				<tr>
					<td colspan="6" class="right-align"><?php echo TwintCw_Language::_('Total Capture Amount'); ?>:</td>
		...
		<?php if ($transaction->getTransactionObject()->isCaptureClosable()):?>
			<div class="closable-box">
				<label for="close-transaction"><?php echo TwintCw_Language::_('Close transaction for further captures'); ?></label>
				<input id="close-transaction" type="checkbox" name="close" value="on" />
			</div>
		<?php endif;?>
		
		<div style="text-align: right;">
			<a onclick="$('#capture-form').submit();" class="btn btn-success"><?php echo TwintCw_Language::_('Capture'); ?> </a>
		</div>
	</form>
<?php endif;?>
